        ...
        <View style={{ padding: 10, backgroundColor: '#ffffff', borderRadius: 10, marginBottom: 10, elevation: 3 }}>
            ...
            <View style={{ paddingVertical: 10, flexDirection: 'row', justifyContent: 'space-between', alignContent: 'center', alignItems: 'center' }}>
                <View style={{ flexDirection: 'row', flex: 1 }}>
                    <Image source={techDetails?.technicianuser?.profilepic ? { uri: techDetails?.technicianuser?.profilepic?.url } : require('../assets/user.png')} resizeMode='contain' style={{ height: 50, width: 50, borderRadius: 25 }} />
                    <View style={{ marginLeft: 10 }}>
                        <Text style={{ color: '#000000', fontSize: 15, marginBottom: 1, fontWeight: '600', width: width / 3.5 }} numberOfLines={1}>{assingedTo?.id ? assingedTo?.firstname + " " + assingedTo?.lastname : 'N/A'}</Text>
                        <Text style={{ color: '#9d9d9d', fontSize: 12, marginBottom: 1, fontWeight: '600', width: width / 3.5 }} numberOfLines={1}>{serviceType}</Text>
                ...
                <View style={{ flex: 1, justifyContent: 'flex-end', alignItems: 'flex-end', alignContent: 'center', borderLeftColor: '#DCEBF7', borderLeftWidth: 2 }}>
                    ...
                    <TouchableOpacity style={{ marginTop: 10, borderColor: '#00B0EB', borderWidth: 1, borderRadius: 8, padding: 5, }} onPress={() => openBrowser(techDetails?.technician?.covidcertificate?.url)}>
